          ...
          <li key={route.href}>
            <Link
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg no-underline font-medium text-sm text-zinc-500 ${
                pathname === route.href
                  ? "bg-zinc-900 px-3 py-2  rounded-xl text-white! transition-colors duration-200" // when active
                  : "hover:text-zinc-900 hover:bg-zinc-100 transition-colors duration-200" // when inactive
              }`}
            >
              {route.icon}
              {route.label}
